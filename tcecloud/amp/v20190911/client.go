...
func NewUpdateShieldRequest() (request *UpdateShieldRequest) {
    request = &UpdateShieldRequest{
        BaseRequest: &tchttp.BaseRequest{},
    }
    request.Init().WithApiInfo("amp", APIVersion, "UpdateShield")
    return
}

func NewUpdateShieldResponse() (response *UpdateShieldResponse) {
    response = &UpdateShieldResponse{
        BaseResponse: &tchttp.BaseResponse{},
    }
    return
}
...
// 修改屏蔽规则
func (c *Client) UpdateShield(request *UpdateShieldRequest) (response *UpdateShieldResponse, err error) {
    if request == nil {
        request = NewUpdateShieldRequest()
    }
    response = NewUpdateShieldResponse()
    err = c.Send(request, response)
    return
}
