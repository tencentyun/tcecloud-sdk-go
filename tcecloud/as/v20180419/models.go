...
type CreateLaunchConfigurationRequest struct {
	*tchttp.BaseRequest
	...
	// 指定有效的镜像ID，格式形如`img-8toqc6s3`。镜像类型分为四种：<br/><li>公共镜像</li><li>自定义镜像</li><li>共享镜像</li><li>服务市场镜像</li><br/>可通过以下方式获取可用的镜像ID：<br/><li>`公共镜像`、`自定义镜像`、`共享镜像`的镜像ID可通过登录控制台查询；`服务镜像市场`的镜像ID可通过云市场查询。</li><li>通过调用接口 DescribeImages ，取返回信息中的`ImageId`字段。</li>
	ImageId *string `json:"ImageId,omitempty" name:"ImageId"`

	// 实例所属项目ID。该参数可以通过调用 DescribeProject 的返回值中的`projectId`字段来获取。不填为默认项目。
	ProjectId *uint64 `json:"ProjectId,omitempty" name:"ProjectId"`

	// 实例机型。不同实例机型指定了不同的资源规格，具体取值可通过调用接口 DescribeInstanceTypeConfigs 来获得最新的规格表或参见实例类型描述。
	...
	// 实例所属安全组。该参数可以通过调用 DescribeSecurityGroups 的返回值中的`SecurityGroupId`字段来获取。若不指定该参数，则默认不绑定安全组。
...
type CreateNotificationConfigurationRequest struct {
	*tchttp.BaseRequest
	...
	// 通知组ID，即为用户组ID集合，用户组ID可以通过DescribeUserGroup查询。
	NotificationUserGroupIds []*string `json:"NotificationUserGroupIds,omitempty" name:"NotificationUserGroupIds" list`
}
...
type CreatePaiInstanceResponse struct {
	*tchttp.BaseResponse
	Response *struct {
		...
		// 当通过本接口来创建实例时会返回该参数，表示一个或多个实例`ID`。返回实例`ID`列表并不代表实例创建成功，可根据 DescribeInstances 接口查询返回的InstancesSet中对应实例的`ID`的状态来判断创建是否完成；如果实例状态由“准备中”变为“正在运行”，则为创建成功。
...
type DataDisk struct {
	...
	// 数据盘类型。数据盘类型限制详见CVM实例配置。取值范围：<br><li>LOCAL_BASIC：本地硬盘<br><li>LOCAL_SSD：本地SSD硬盘<br><li>CLOUD_BASIC：普通云硬盘<br><li>CLOUD_PREMIUM：高性能云硬盘<br><li>CLOUD_SSD：SSD云硬盘<br><br>默认取值：LOCAL_BASIC。
...
type NotificationTarget struct {
	...
	// <li> CMQ_QUEUE，指Tce消息队列-队列模型。</li>
	// <li> CMQ_TOPIC，指Tce消息队列-主题模型。</li>
...
type SystemDisk struct {
	...
	// 系统盘类型。系统盘类型限制详见CVM实例配置。取值范围：<br><li>LOCAL_BASIC：本地硬盘<br><li>LOCAL_SSD：本地SSD硬盘<br><li>CLOUD_BASIC：普通云硬盘<br><li>CLOUD_PREMIUM：高性能云硬盘<br><li>CLOUD_SSD：SSD云硬盘<br><br>默认取值：LOCAL_BASIC。
	// 注意：此字段可能返回 null，表示取不到有效值。
	DiskType *string `json:"DiskType,omitempty" name:"DiskType"`
