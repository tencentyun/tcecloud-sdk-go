...
type AddMountTargetRequest struct {
	*tchttp.BaseRequest
	...
	// 指定 IP 地址，该字段留空则将随机分配 IP
	MountIP *string `json:"MountIP,omitempty" name:"MountIP"`
}

func (r *AddMountTargetRequest) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}

func (r *AddMountTargetRequest) FromJsonString(s string) error {
    return json.Unmarshal([]byte(s), &r)
}

type AddMountTargetResponse struct {
	*tchttp.BaseResponse
	Response *struct {
...
func (r *AddMountTargetResponse) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}

func (r *AddMountTargetResponse) FromJsonString(s string) error {
    return json.Unmarshal([]byte(s), &r)
}
